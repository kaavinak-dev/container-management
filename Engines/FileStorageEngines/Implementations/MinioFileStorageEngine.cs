...
{
    public class MinioFileStorageEngine : FileStorageEngine
    {
        ...
        public MinioFileStorageEngine(string engineUrl, int enginePort) : base(engineUrl, enginePort)
        {
        ...
        public override async Task<FileContainer> UploadRawBinary(Stream fileData, string bucketName = "quarantine")
        {
            try
            {
                ...
                return new ExecutableFileContainer(objectName, bucketName, this.GetServerUrl());
            }
            catch (Exception ex)
            {
        ...
        public async Task<Stream> DownloadFile(string bucketName, string fileName)
        {
            try
            {
                ...
                Console.WriteLine($"Downloaded file: {fileName} from bucket: {bucketName}");
            ...
            {
                Console.WriteLine($"Error downloading file {fileName} from bucket {bucketName}: {ex.Message}");
                throw;
            }
        }
